...
[changelog.md](changelog.md) regenerates on every `git commit` and `git push` (local hooks plus GitHub Actions). Run `npm install` once so `prepare` installs the hooks. Do not edit the generated changelog by hand; older session notes are in [docs/changelog-history.md](docs/changelog-history.md). Keep [README.md](README.md) as product documentation — hooks do not rewrite it.
