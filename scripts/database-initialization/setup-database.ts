...
import { setupGithubEvents } from './collections/github-events';
import { setupGithubCommits } from './collections/github-commits';
import { setupGithubPullRequests } from './collections/github-pull-requests';
import { setupGithubIssues } from './collections/github-issues';
import { setupGithubReleases } from './collections/github-releases';
...
const collectionSetups: Array<{
    ...
    setup: (databases: import('node-appwrite').Databases, databaseId: string) => Promise<void>;
        ...
        { name: 'GitHub Events', setup: setupGithubEvents },
        { name: 'GitHub Commits', setup: setupGithubCommits },
        { name: 'GitHub Pull Requests', setup: setupGithubPullRequests },
        { name: 'GitHub Issues', setup: setupGithubIssues },
        { name: 'GitHub Releases', setup: setupGithubReleases },
        ...
        { name: 'Agent Runs', setup: setupAgentRuns },
        { name: 'Agent Harness', setup: setupAgentHarness },
        { name: 'Personal Agents', setup: setupPersonalAgents },
        { name: 'Agent Jobs', setup: setupAgentJobs },
    ];
...
function getAutoGeneratedVars(): Record<string, string> {
    return {
        ...
        NEXT_PUBLIC_AI_MODEL_PRICING_ID: process.env.NEXT_PUBLIC_AI_MODEL_PRICING_ID || 'ai_model_pricing',
        ...
        NEXT_PUBLIC_APPWRITE_GITHUB_ACCOUNTS_ID: process.env.NEXT_PUBLIC_APPWRITE_GITHUB_ACCOUNTS_ID || 'github_accounts',
        NEXT_PUBLIC_APPWRITE_GITHUB_EVENTS_ID: process.env.NEXT_PUBLIC_APPWRITE_GITHUB_EVENTS_ID || 'github_events',
        NEXT_PUBLIC_APPWRITE_CODE_DOCS_ID: process.env.NEXT_PUBLIC_APPWRITE_CODE_DOCS_ID || 'code_documentation',
        // This is the new one added (Slack/Discord/MCP/GitLab/Bitbucket)
        NEXT_PUBLIC_APPWRITE_PROJECT_INTEGRATIONS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_PROJECT_INTEGRATIONS_ID || 'project_integrations',
        NEXT_PUBLIC_APPWRITE_MCP_API_TOKENS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_MCP_API_TOKENS_ID || 'mcp_api_tokens',
        NEXT_PUBLIC_APPWRITE_AGENT_MCP_CONFIGS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_AGENT_MCP_CONFIGS_ID || 'agent_mcp_configs',
        NEXT_PUBLIC_APPWRITE_AGENT_RUNS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_AGENT_RUNS_ID || 'agent_runs',
        NEXT_PUBLIC_APPWRITE_AGENT_HARNESS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_AGENT_HARNESS_ID || 'agent_harness',
        NEXT_PUBLIC_APPWRITE_AGENT_JOBS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_AGENT_JOBS_ID || 'agent_jobs',
        NEXT_PUBLIC_APPWRITE_AGENT_AI_CONFIGS_ID:
            process.env.NEXT_PUBLIC_APPWRITE_AGENT_AI_CONFIGS_ID || 'agent_ai_configs',
        NEXT_PUBLIC_APPWRITE_PERSONAL_AGENTS_ID:
        ...
        NEXT_PUBLIC_APPWRITE_BUG_REPORTS_BUCKET_ID: process.env.NEXT_PUBLIC_APPWRITE_BUG_REPORTS_BUCKET_ID || 'bug_reports_bucket',

        // ── BYOB ──
        NEXT_PUBLIC_APPWRITE_BYOB_TENANTS_ID: process.env.NEXT_PUBLIC_APPWRITE_BYOB_TENANTS_ID || 'byob_tenants',

        // ── Bug Reports ──
        NEXT_PUBLIC_APPWRITE_BUG_REPORTS_ID: process.env.NEXT_PUBLIC_APPWRITE_BUG_REPORTS_ID || 'bug_reports',

        // ── Billing Defaults ──
        BILLING_GRACE_PERIOD_DAYS: process.env.BILLING_GRACE_PERIOD_DAYS || '7',
        BILLING_CURRENCY: process.env.BILLING_CURRENCY || 'USD',
        USAGE_RATE_TRAFFIC_GB: process.env.USAGE_RATE_TRAFFIC_GB || '12.0',
        USAGE_RATE_STORAGE_GB_MONTH: process.env.USAGE_RATE_STORAGE_GB_MONTH || '2.0',
        USAGE_RATE_COMPUTE_UNIT: process.env.USAGE_RATE_COMPUTE_UNIT || '1.0',
...
function getUserRequiredVars(): EnvVarEntry[] {
    return [
        ...
        },
        {
            ...
            description: 'GitHub OAuth Client ID → Register OAuth App in GitHub Settings > Developer Settings',
        },
        {
            key: 'GITHUB_CLIENT_SECRET',
            value: process.env.GITHUB_CLIENT_SECRET || '',
            description: 'GitHub OAuth Client Secret → Register OAuth App in GitHub Settings > Developer Settings',
        },
        {
            key: 'INTEGRATION_ENCRYPTION_SECRET',
            value: process.env.INTEGRATION_ENCRYPTION_SECRET || '',
            description: 'Encryption key for OAuth tokens at rest (generate a 32-char hex string: openssl rand -hex 16)',
        },
