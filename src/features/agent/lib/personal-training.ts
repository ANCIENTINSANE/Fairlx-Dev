...
export function buildTrainingInterviewPrompt(input: {
  ...
  return [
    ...
    covered.length ? "" : "- End that first message by calling ask_user with 3 invented role choices, including an infer/skip path and a beginner path. Do not use a fixed list.",
    ...
    "- For every question, call ask_user with 3 short invented option labels for THAT question — role guesses, skip, infer from workspace/team, beginner, or a concrete snapshot-based answer. Never reuse a fixed hardcoded set.",
    "- ask_user always adds Type your own as the last option. Do not add an Other option. Do not mention the tool name.",
    ...
    "- Option titles must be self-contained replies under 40 characters. Optional short description after an em dash.",
