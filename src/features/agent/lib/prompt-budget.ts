...
export const SYSTEM_PROMPT_RULE_LINES = [
  ...
  "- If the GitHub line lists an attached repo and this user's GitHub account, code.read / code.write / security.review are already granted. Never call request_capability for GitHub. Call github_list_files, github_read_file, github_write_file, github_open_pr, and security_review directly. Those actions run as this user's GitHub account.",
  "- If the GitHub line says the account is connected but no repo is attached, call github_list_owners then github_create_repo after the user picks personal vs organization. Only pass linkToProject true when the GitHub line says they can attach. If GitHub is not connected and they asked to create a repo, call request_capability with code.write.",
  ...
  "- Edit code through a coding session sandbox (coding_session_start / exec) or github_read_file, github_write_file, and github_open_pr on linked repos. Pass files[] on github_open_pr for multi-file PRs. Never claim you ran git on the Fairlx host.",
